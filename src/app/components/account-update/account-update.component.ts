...
export class AccountUpdateComponent implements OnInit {
  ...
  customers:Customer[]=[];
  constructor(private formBuilder:FormBuilder,private accountService:AccountService,private toastrService:ToastrService,
    private customerService:CustomerService, private timerService:TimerService) { }
  ...
  update(){
    if(this.accountUpdateForm.valid){
      let accountUpdateModel= Object.assign({id:this.id},this.accountUpdateForm.value)
      
      this.accountService.update(accountUpdateModel).subscribe(response=>{
        this.toastrService.success(response.message,"Hesap Güncelleme")
  ...
  refresh(){
    this.timerService.wait(this.pageReload, 4000)
  }
}